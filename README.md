...
{ h:'handle',        // X 用户名，不带 @（必填）
  ...
  c:['flj','photo'], // 分类：flj=福利姬, photo=私密摄影, indie=独立开发, sea=出海, cos=COS, acg=二次元, ai=AI（可多选）
...
- **每周更新一次**：改数据时同步更新 `META.updated` 日期，页面顶部的周数徽章（"2026 年第 N 周榜 · MM.DD – MM.DD"）会按该日期自动计算 ISO 周数和周一~周日区间，无需手动维护。
